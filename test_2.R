...
CD4<-bh_defineMarker(markerName = 'cd4',
                     ...
                     patternModifier = list(mean = 2, sd=3,k=5,X0=0.1),
                     compartment = 'cytoplasm')
...
cell2<-bh_defineCell(cytoplasm = cytop1,
                     nucleus = nuc1,
                     organelle = NULL,
                     markers = list(CD45,CD4,DNA,CD1000high))

cell3<-bh_defineCell(cytoplasm = cytop1,
                     nucleus = nuc1,
                     organelle = NULL,
                     markers = list(CD45,CD8,DNA,CD1000Low))
cell4<-bh_defineCell(cytoplasm = cytop1,
                     nucleus = nuc1,
                     organelle = NULL,
                     markers = list(CD45,CD8,DNA,CD1000high))

cell5<-bh_defineCell(cytoplasm = cytop2,
                     nucleus = nuc1,
                     organelle = NULL,
...
TEMP_population<-bh_populate(cellPrototype = list(cell1,cell2,cell3,cell4,cell5),
                  cellNames = c('CD4lo','CD4hi','CD8lo','CD8hi','dend'),
...
TEMP_pic<-bh_engrave(tissue = tissue1,
                  cells = TEMP_population$CD4hi)
TEMP_pic<-bh_engrave(tissue = TEMP_pic,
                  cells = TEMP_population$CD4lo)
...
TEMP_mod<-bh_modifier(TEMP_pic$x.ch1.ch1,wMatrix = matrix(1,11,11),fun=.modifier.scramble)
TEMP_mod<-bh_modifier(TEMP_mod,wMatrix = .matrix.gauss(TEMP_mod,5),fun = mean)

TEMP_mod<-bh_modifier(TEMP_pic$x.ch1.ch1,wMatrix =  matrix(c(1),11,11),fun = .modifier.addNoise)
